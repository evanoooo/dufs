<template>
    ...
    <v-card class="my-4" :style="glassmorphism.filelist">
            ...
            <v-breadcrumbs :items="breadcrumb" class="flex-grow-1 overflow-x-auto my-0 py-2 py-sm-4">
                ...
                <template v-slot:prepend>
                    ...
                    <v-btn
                        ...
                        icon="$mdiTrayArrowUp"
                        density="comfortable"
                        :to="breadcrumb.length > 1 ? breadcrumb[breadcrumb.length - 2].href : pathPrefix"
        ...
        >
                        ...
                        <td>
                            ...
                            >
                                ...
                                <a
                                    ...
                                    class="text-decoration-none"
                                    style="color:inherit;overflow-wrap:anywhere"
                                    @click="e => filenameClickPreview(p, e)"
                                >{{ p.name }}</a>
                                <v-icon
                                    v-if="p.is_symlink"
                                    icon="$mdiLinkVariant"
            ...
            <div
                v-if="!filelistPathsSorted.length"
                class="d-flex flex-column justify-center align-center px-4 py-16 text-disabled"
            >
                <v-icon icon="$mdiPackageVariant" size="72"></v-icon>
                <span class="mt-4">{{ t('toastEmpty') }}</span>
            </div>
        </v-skeleton-loader>
    </v-card>
...
const preferPreview = window.__DUFS_MATERIAL_CONFIG__?.preferPreview;
const codeHighlightMaxLines = window.__DUFS_MATERIAL_CONFIG__?.codeHighlightMaxLines;
const codeHighlightMaxLineLength = window.__DUFS_MATERIAL_CONFIG__?.codeHighlightMaxLineLength;
...
new Promise(resolve => {
    ...
    image.onload = () => {};
    image.onerror = () => {
        if (!window.__DUFS_MATERIAL_CONFIG__?.jxlPolyfill) {
            previewableImageExts.delete('jxl');
            previewablePhotopeaExts.add('jxl');
            return;
        }
        const unwatchHandle = watch(previewItem, () => {
            if (previewMode.value === 'image' && previewItem.value.ext === 'jxl') {
                /* @vite-ignore */
                import(
                    typeof window.__DUFS_MATERIAL_CONFIG__?.jxlPolyfill === 'string'
                        ? window.__DUFS_MATERIAL_CONFIG__?.jxlPolyfill
                        : 'https://cdn.jsdelivr.net/npm/jxl-rs-polyfill/dist/jxl-polyfill.min.js'
                ).then(({ JXLPolyfill }) => {
...
const updatePreviewContent = async () => {
    ...
    if (previewItem.value.ext === 'md') {
        ...
        const lineLengths = r.split('\n').map(e => e.trim().length);
        const skipHighlight = (
            (codeHighlightMaxLines && lineLengths.length > codeHighlightMaxLines) ||
            (codeHighlightMaxLineLength && Math.max.apply(null, lineLengths) > codeHighlightMaxLineLength)
        );
        const lang = skipHighlight ? 'plain' : codeLanguageTable[previewItem.value.ext];
 ...
 */
const filenameClickPreview = (p, e) => {
    if (!preferPreview) return;
    previewItem.value = null;
    if (previewableImageExts.has(p.ext)) {
        previewMode.value = 'image';
        previewItem.value = p;
    } else if (previewableVideoExts.has(p.ext)) {
        previewMode.value = 'video';
        previewItem.value = p;
        updateVideoSubtitles(p);
    } else if (previewableAudioExts.has(p.ext)) {
        ...
        previewItem.value = p;
    } else if (previewableTextFilenames.has(p.filename.toLowerCase()) || previewableTextExts.has(p.ext) || codeLanguageTable[p.ext]) {
        previewMode.value = 'text';
        previewItem.value = p;
        previewContent.value = '';
        updatePreviewContent();
    } else if (reviewableFontExts.has(p.ext)) {
        previewMode.value = 'font';
        previewItem.value = p;
    } else if (externalViewer) {
        if (previewableOfficeExts.has(p.ext) && p.size <= 10485760 && !isLocal) {
            previewMode.value = 'office';
            previewItem.value = p;
        } else if (previewableDrawioExts.has(p.ext) && !isLocal) {
            previewMode.value = 'drawio';
            previewItem.value = p;
        } else if (previewablePhotopeaExts.has(p.ext)) {
            previewMode.value = 'photopea';
            previewItem.value = p;
        }
    }
